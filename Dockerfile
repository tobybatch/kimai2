...
FROM php:7.3.16-fpm-alpine3.10 AS fpm-alpine-php-ext-base
...
FROM php:7.3.16-apache-buster AS apache-debian-php-ext-base
RUN apt-get update
...
# fpm-alpine base build
FROM php:7.3.16-fpm-alpine3.10 AS fpm-alpine-base
RUN apk add --no-cache \
        bash \
        freetype \
        haveged \
...
FROM php:7.3.16-apache-buster AS apache-debian-base
