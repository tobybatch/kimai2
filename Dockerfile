...
FROM php:8.2.0RC1-fpm-alpine3.15 AS fpm-php-ext-base
...
FROM php:8.1.0-apache-buster AS apache-php-ext-base
RUN apt-get update
...
# fpm base build
FROM php:8.2.0RC1-fpm-alpine3.15 AS fpm-base
RUN apk add --no-cache \
        bash \
        coreutils \
        freetype \
...
FROM php:8.1.0-apache-buster AS apache-base
