...
FROM php:8.0-fpm-alpine3.13 AS fpm-alpine-php-ext-base
...
FROM php:8.0-fpm-alpine3.13 AS apache-debian-php-ext-base
RUN apt-get update
...
# fpm-alpine base build
FROM php:8.0-fpm-alpine3.13 AS fpm-alpine-base
RUN apk add --no-cache \
        bash \
        freetype \
        haveged \
...
FROM php:8.0-fpm-alpine3.13 AS apache-debian-base
